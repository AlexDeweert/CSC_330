...
if not (a.x == THREE and a.y == FIVE)
    puts "#{__LINE__}: Point is not initialized properly"
end
if not (a.eval_prog([]) == a)
    puts "#{__LINE__}: Point eval_prog should return self"
end
if not (a.preprocess_prog == a)
    puts "#{__LINE__}: Point preprocess_prog should return self"
end
...
if not (b.m == THREE and b.b == FIVE)
    puts "#{__LINE__}: Line not initialized properly"
end
if not (b.eval_prog([]) == b)
    puts "#{__LINE__}: Line eval_prog should return self"
end
if not (b.preprocess_prog == b)
    puts "#{__LINE__}: Line preprocess_prog should return self"
end
b1 = b.shift(THREE,FIVE)
if not (b1.m == THREE and b1.b == ONE)
    puts "#{__LINE__}: Line shift not working properly"
end
...
if not (c.x == THREE)
    puts "#{__LINE__}: VerticalLine not initialized properly"
end
if not (c.eval_prog([]) == c)
    puts "#{__LINE__}: VerticalLine eval_prog should return self"
end
if not (c.preprocess_prog == c)
    puts "#{__LINE__}: VerticalLine preprocess_prog should return self"
end
...
if not (d.eval_prog([]) == d)
    puts "#{__LINE__}: LineSegement eval_prog should return self"
end
d1 = LineSegment.new(ONE,TWO,ONE,TWO)
d2 = d1.preprocess_prog
if not ((d2.is_a? Point)and d2.x == ONE and d2.y == TWO)
    puts "#{__LINE__}: LineSegment preprocess_prog should convert to a Point"
    puts "#{__LINE__}: if ends of segment are real_close"
end

d = d.preprocess_prog
if not (d.x1 == -THREE and d.y1 == -FOUR and d.x2 == ONE and d.y2 == TWO)
    puts "#{__LINE__}: LineSegment preprocess_prog should make x1 and y1"
    puts "#{__LINE__}: on the left of x2 and y2"
end

d3 = d.shift(THREE,FIVE)
if not (d3.x1 == ZERO and d3.y1 == ONE and d3.x2 == FOUR and d3.y2 == SEVEN)
    puts "#{__LINE__}: LineSegment shift not working properly"
end
...
if not ((v1.is_a? Point) and v1.x == THREE and v1.y == FIVE)
    puts "#{__LINE__}: Var eval_prog is not working properly"
end
if not (v1.preprocess_prog == v1)
    puts "#{__LINE__}: Var preprocess_prog should return self"
end
...
#puts ">#{s1.class} #{s1.inspect}"
if not (s1.x1 == TWO and s1.y1 == THREE and s1.x2 == SIX and s1.y2 == 9)
    puts "#{__LINE__}: Shift should shift e by dx and dy"
end
...
#puts ">#{LineSegment.new(TWO, ZERO, ONE, ONE).inspect}"
#puts ">#{LineSegment.new(TWO, ZERO, ONE, ONE).preprocess_prog.inspect}"
